...
echo "== Entitlements =="
# App Sandbox must NOT be present — CGEvent tap requires Accessibility permission,
# which is unavailable inside App Sandbox.
...
if echo "$entitlements_output" | grep -q "com.apple.security.app-sandbox"; then
  fail "App Sandbox entitlement found — unexpected for this app (CGEvent tap requires no sandbox)."
else
  pass "No App Sandbox entitlement (correct for CGEvent tap)."
fi
